...
def test_shadow_mirror_imports_native_planes_and_preserves_provenance(
    ...
    assert (
        Path(manager.state().trial_dir) / NATIVE_BASELINE_MANIFEST_NAME
    ).is_file()
...
def test_shadow_mirror_resynchronizes_when_native_memory_changes(
    ...
    assert second["native_baseline"]["snapshot_sha256"] == first["native_baseline"][
        "snapshot_sha256"
    ]
...
def test_takeover_freezes_native_files_and_rollback_restores_them(
    ...
    with (workspace / "memory" / "2026-07-30.md").open(
        "a", encoding="utf-8"
    ) as sink:
    ...
    assert configured["plugins.entries.memory-aetnamem.config.dbPath"] == mirror[
        "mirror_db"
    ]
    ...
    assert "lossless snapshots" in (
        workspace / "memory" / "2026-07-30.md"
    ...
    snapshot = json.loads(
        (
            Path(manager.state().trial_dir) / NATIVE_SNAPSHOT_MANIFEST_NAME
        ).read_text(encoding="utf-8")
    )
    assert snapshot["file_count"] == 5
    assert snapshot["snapshot_sha256"]
    assert all(row.get("sha256") for row in snapshot["entries"] if row["type"] == "file")
    cutover = json.loads(
