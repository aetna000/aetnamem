...
This paper documents the architecture implemented in the open-source \system{} release v0.3.0, at commit \code{69ebedea93ea95965867af5483dd9f63425b8e75}~\cite{aetnamem2026}. It makes five scoped contributions.
...
\begin{enumerate}
  ...
  \item It binds memory, graph, retrieval, and effect transitions into one per-subject hash chain with externally anchorable checkpoints and crash-aware action states.
...
Non-specialist readers can follow the plain-language boxes, \cref{fig:architecture}, the lifecycle table in \cref{tab:lifecycle}, and the platform matrix in \cref{tab:platform-encryption}. Technical readers will find the scoring equations, graph bounds, transaction model, cryptographic construction, action state machine, implementation map, and reproducibility details in the main text and appendix.
