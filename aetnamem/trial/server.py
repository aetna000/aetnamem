...
from aetnamem.mcp.server import PROTOCOL_VERSION, SERVER_VERSION
from aetnamem.trial.manager import TrialManager


class TrialMCPServer:
    def __init__(self, manager: TrialManager) -> None:
    ...
    def _call(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        elif name == "trial_exposure_shown":
            value = {
                "confirmed": self.manager.confirm_exposure(
                    str(arguments["exposure_id"])
                )
            }
        elif name == "trial_status":
            value = self.manager.status()
        else:
            raise ValueError(f"unknown trial tool: {name}")
        return {
            "content": [
...
def _tools() -> list[dict[str, Any]]:
    return [
        ...
        {
            "name": "trial_prepare",
            "description": "Build a preview and inject it only when trial mode permits.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "query": {"type": "string"},
        ...
        {
            "name": "trial_status",
            "description": "Read fail-closed trial status and evidence counts.",
            "inputSchema": {
                "type": "object",
                "properties": {},
                "additionalProperties": False,
