...
class OllamaProvider:
    """Local model provider for Ollama's loopback chat API."""
    ...
    def complete(self, messages: list[dict[str, str]], tools: list[dict[str, Any]]) -> str:
        url = (self.config.base_url or DEFAULT_OLLAMA_BASE_URL).rstrip("/")
        body = {
            "model": self.config.model or DEFAULT_LOCAL_MODEL,
            "messages": [
                *messages,
                {
                    "role": "system",
        ...
        try:
            ...
            raise RuntimeError(
                "local model is not reachable; start Ollama and pull "
                f"{self.config.model or DEFAULT_LOCAL_MODEL}"
            ) from exc
        message = payload.get("message") or {}
        content = message.get("content")
        if not isinstance(content, str):
            raise RuntimeError("local model returned an invalid chat response")
        return content
...
def provider_from_config(config: ProviderConfig) -> AssistantProvider:
    if config.kind == "echo":
        return EchoProvider()
    if config.kind in {"local", "ollama"}:
...
def config_from_env() -> ProviderConfig:
    ...
    if provider in {"local", "ollama"}:
        return ProviderConfig(
            kind="local",
            model=os.environ.get("AETNAMEM_LOCAL_MODEL", DEFAULT_LOCAL_MODEL),
            base_url=os.environ.get("AETNAMEM_OLLAMA_URL", DEFAULT_OLLAMA_BASE_URL),
        )
    return ProviderConfig()
