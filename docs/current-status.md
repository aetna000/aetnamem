...
|---|---|---|
| Python `Memory`, CLI, and 19-tool MCP | Released core plus experimental compatibility additions | The v0.6.1 surface remains supported; `memory_get_record` and `memory_get_source` are additive in v0.6.1.1a5 |
| Verified OpenClaw installer | Experimental preview (`v0.6.1.1a5`) | Engine-owned bridge install/upgrade, absolute executable binding, native-memory shadow start, gateway restart/RPC probe, retained-config verification, and rollback on failure |
| Safe Switch control plane | Experimental preview (`v0.6.1.1a5`) | Customer surface has two states: OpenClaw active while AetnaMem mirrors without changing context, or AetnaMem active; rollback restores OpenClaw. Detailed retrieval/exposure events remain internal audit evidence rather than customer modes |
...
| OpenClaw native-memory mirror | Experimental preview (`v0.6.1.1a5`) | Verified byte-for-byte pre-shadow baseline, observed native-state versions, hash-bound Markdown import with source-file and line provenance, isolated SQLite mirror, status/search/trace |
| OpenClaw memory takeover | Experimental preview (`v0.6.1.1a5`) | Final mirror verification, complete switch-time snapshot, standard `memory_search` / `memory_get` compatibility, audited exact reads, unsupported-capability gate, runtime tool/hook verification, and hash-verified rollback |
...
| Safe Switch dashboard | Experimental preview (`v0.6.1.1a5`) | One functional page: active provider, exact mirrored source manifest, ordinary-language memory search, verification checks, and activate/restore control. No preview, canary, emergency, candidate queue, or mock comparison UI. Loopback-only API, one-time login, HttpOnly cookie, CSRF and origin controls remain |
| Dashboard daemon | Experimental preview (`v0.6.1.1a5`) | Detached local start/stop/restart/status/remove lifecycle with PID validation; restarts ignore consumed login URLs already present in the append-only log; removal preserves memory and evidence |
...
| Current experimental preview notes | [`releases/v0.6.1.1a5.md`](releases/v0.6.1.1a5.md) |
