...
|---|---|---|
| Python `Memory`, CLI, and 19-tool MCP | Released core plus experimental compatibility additions | The v0.6.1 surface remains supported; `memory_get_record` and `memory_get_source` are additive in v0.7.0a6 |
| Verified OpenClaw installer | Experimental preview (`v0.7.0a6`) | Engine-owned bridge install/upgrade, absolute executable binding, staged terminal progress, native-memory shadow start, gateway restart/RPC probe, retained-config verification, and rollback on failure |
| Safe Switch control plane | Experimental preview (`v0.7.0a6`) | Customer surface has two states: OpenClaw active while AetnaMem mirrors OpenClaw's own native-memory writes without parsing chat transcripts, or AetnaMem active; rollback restores OpenClaw. Detailed retrieval/exposure events remain internal audit evidence rather than customer modes |
| Safe Switch OpenClaw adapter | Experimental preview (`npm v0.5.0-experimental.4`) | Private host protocol, startup MCP handshake, silent shadow recall, no agent memory tools during trial, verified native-memory takeover and rollback; active takeover adds model-semantic `memory_remember`, a short-lived cross-runtime binding to OpenClaw's typed current-prompt event, and an enforced guard for the exact frozen native-memory paths. It does not scrape transcripts or use keyword capture in active mode |
| OpenClaw native-memory mirror | Experimental preview (`v0.7.0a6`) | Verified byte-for-byte pre-shadow baseline, observed native-state versions, hash-bound Markdown import with source-file and line provenance, isolated SQLite mirror, status/search/trace |
| OpenClaw memory takeover | Experimental preview (`v0.7.0a6`) | Final mirror verification, complete switch-time snapshot, standard `memory_search` / `memory_get` compatibility, audited exact reads, unsupported-capability gate, verified native-path tool guard, active-period memory export, divergent-file preservation, hash-verified rollback, readiness-checked activation from legacy `off`, repeat activation after completed rollback, and explicit interrupted-cutover recovery |
...
| Safe Switch dashboard | Experimental preview (`v0.7.0a6`) | One functional page: active provider, focused memory search, exact mirrored source manifest, typed-observation approval, verification checks, visible progress, and activate/restore control. Image reviews place the digest-verified OpenClaw source beside the exact description that would become recallable. Record IDs open an auditor drawer; the global Audit Explorer adds compound filters, histogram, cursor pagination, pivots, saved local views and CSV/JSON/NDJSON/text export. Missing historic evidence is labeled `not recorded`. Loopback-only API, HttpOnly cookie, CSRF and origin controls remain |
| Dashboard daemon | Experimental preview (`v0.7.0a6`) | Detached local start/open/stop/restart/status/remove lifecycle with PID validation; one mode-`0600` protected access key survives normal restart, OS-native open bypasses editor link handlers, and startup fails closed without a URL. Removal deletes the service record and access key while preserving memory and evidence |
...
| Current experimental preview notes | [`releases/v0.7.0a6.md`](releases/v0.7.0a6.md) |
